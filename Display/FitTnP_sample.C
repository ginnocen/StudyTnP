...
void FitTnP_sample(){
  ...
  RooRealVar mass("mass", "mass", mumulow, mumuhigh);
 ...
 // const double MuPtBin[nMuPtBin+1] = {0.0, 1.5, 3.0, 4.5, 6.0, 9.0, 20.0, 30.0};
  ...
  //get pt histogram
  hTrkPtPass = (TH1D*)finput->Get("hTrkPtPass3");
  hTrkPtFail = (TH1D*)finput->Get("hTrkPtFail3");
  //hTrkPtAll = (TH1D*)finput->Get("hTrkPtAll3");
  ...
  //RooRealVar cheb_p2All("cheb_p2All", "cheb_p2All", 0., -1., +1.);
  ...
  //set axis variable
  RooRealVar xtrk("xtrk", "mass", mumutrklow, mumutrkhigh, "GeV");
  ...
  //RooDataHist dataTrkAll("dataTrkAll", "dataTrkAll", xtrk, hTrkPtAll);
  ...
  dataTrkAllset.plotOn(frame1, Cut("sampleTrk==sampleTrk::Pass"), Binning(100));
  ...
  dataTrkFail.plotOn(frame2, Binning(100));

  simPdf.plotOn(frame2, Slice(sampleTrk, "Fail"), Components("signal_gaus_Fail"),ProjWData(sampleTrk,dataTrkFail),LineStyle(kDashed), LineColor(kGreen));
  simPdf.plotOn(frame2, Slice(sampleTrk, "Fail"), Components("background_cheb_Fail"),ProjWData(sampleTrk,dataTrkFail),LineStyle(kDashed), LineColor(kRed));

  cfail->cd();
  frame2->Draw();

  //RooPlot *frame3 = mass.frame();
  //dataTrkAll.plotOn(frame3, Binning(100));

  //simPdf.plotOn(frame3, Slice(sampleTrk, "All"), Components("signal_gaus_All"),ProjWData(sampleTrk,dataTrkAll),LineStyle(kDashed), LineColor(kGreen));
  //simPdf.plotOn(frame3, Slice(sampleTrk, "All"), Components("background_cheb_All"),ProjWData(sampleTrk,dataTrkAll),LineStyle(kDashed), LineColor(kRed));
